...
function do_summarize_artefactrejection(subjectlist, data_dir)
% This script should be run when the artefact rejection is complete. 
% It will incorporate the rejected channels and trials in a derivatives
% subfolder

% Specifically, number of rejected trials and excluded participants will go in a participants tsv

% Begsample, endsample, and round of artefact rejection of each rejected
% trial go in a badtrials tsv file

% Rejected channels go in a channels tsv

% We create a derivatis_dir
derivatives_dir_temp      = erase(data_dir, 'BIDS');
derivatives_dir           = [derivatives_dir_temp 'derivatives'] ;
if ~exist(derivatives_dir)
...
for ii=1:size(subjectlist,1)
    ...
    if exist([output_dir filesep 'badtrials.mat'], 'file')
        ...
        write_tsv([derivatives_dir filesep 'participants.tsv'], participants);
        ...
        if ~isfield(participants_json, 'included')
            ...
            write_json([derivatives_dir filesep 'participants.json'], participants_json);
        end
        ...
        if ~isempty(badtrials)
            ...
            if exist([derivatives_dir filesep sub filesep 'eeg' filesep sub '_badtrials.tsv'], 'file')
                % The file already exists: we update it
                write_tsv([derivatives_dir filesep sub filesep 'eeg' filesep sub '_badtrials.tsv'], badtrials);
            else
                % We have to make the folder and file
                mkdir([derivatives_dir filesep sub filesep 'eeg'])
                write_tsv([derivatives_dir filesep sub filesep 'eeg' filesep sub '_badtrials.tsv'], badtrials);
                ...
                write_json([derivatives_dir filesep sub filesep 'eeg' filesep sub '_badtrials.json'], badtrials_json);
            end 
        end
    ...
    if exist([output_dir filesep 'badchannelnumbers.mat'], 'file')
        ...
        if ~exist([derivatives_dir filesep sub filesep 'eeg'])
            mkdir([derivatives_dir filesep sub filesep 'eeg']);
        end
        ...
        % Then we update the current tsv file
        write_tsv([derivatives_dir filesep sub filesep 'eeg' filesep sub '_channels.tsv'], channels_tsv);
    else
        % do nothing            
    end       
end
